...
class AffinityMolDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        smi = self.dataset[index]["smi" if "smi" in self.dataset[index] else "smiles"]
        return {
            "atoms": atoms,
            "coordinates": coordinates.astype(np.float32),
            # "holo_coordinates": coordinates.astype(np.float32),#placeholder
...
class AffinityPocketDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        return {
            ...
            "pocket": pocket,
            "ori_length": ori_length,
            "other_labels": self.dataset[index]["other_labels"] if "other_labels" in self.dataset[index] else {},
        }
